...
if (plot_data == 1){
  ...
  if (ENG!=1){
    temp_main_label <- "L452R陽性率（東京、週次）"
  }
  ...
  if (ENG==1){
    ...
    temp_main_label <- "ワクチン接種率（全国、日次）"
    temp_x_lab <- "2021年の日付"
    temp_y_lab <- "割合" 
    temp_legend <- c("1回接種", "2回接種")
  }
...
if(estim==1 || plot_estim_rslt ==1){
  ...
  for (i in 1:(length(temp_nl))){
    ...
    if (ENG==1){
      ...
      legend("topright",legend=c("観測値", "平均値の事後平均", "95%予測区間"), lty=c(1,1,2), col=c(1,2,2))
    }
  }
